...
public class Conversion extends SingleOperation {
    ...
    private final int[] elementMax = {1, Integer.MAX_VALUE, Integer.MAX_VALUE, 1, Integer.MAX_VALUE,
    ...
    public Conversion(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.Conversion);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.Conversion);
                if (!tempList.contains(elementName[i])) {
