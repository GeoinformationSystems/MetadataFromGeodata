...
@XmlRootElement(name = "DatumEnsemble")
public class DatumEnsemble extends ObjectUsage {
    ...
    // class variables
    @XmlElementWrapper(name = "ensembleAccuracy")
    @XmlElementRef
    public List<DQ_PositionalAccuracy> ensembleAccuracy;
    ...
    public DatumEnsemble(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.ObjectDomain);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.ObjectDomain);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addEnsembleAccuracy(DQ_PositionalAccuracy ensembleAccuracy) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.ensembleAccuracy.add(ensembleAccuracy);
            }
