...
public class OperationMethod extends IdentifiedObject {
    ...
    @XmlElementWrapper(name = "formulaReference")
    @XmlElementRef
    public List<Formula> formulaReference;
    ...
    public OperationMethod(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.OperationMethod);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.OperationMethod);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addFormulaReference(Formula formulaReference) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.formulaReference.add(formulaReference);
            }
        } catch (MaximumOccurrenceException | NoSuchFieldException | IllegalAccessException e) {
            System.out.println(e.getMessage());
        }
