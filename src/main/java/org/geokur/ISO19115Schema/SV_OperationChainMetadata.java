...
public class SV_OperationChainMetadata {

    // occurrence and obligation
    private final String[] elementName = {"name", "description", "operation"};
    ...
    public SV_OperationChainMetadata(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.SV_OperationChainMetadata);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.SV_OperationChainMetadata);
                if (!tempList.contains(elementName[i])) {
