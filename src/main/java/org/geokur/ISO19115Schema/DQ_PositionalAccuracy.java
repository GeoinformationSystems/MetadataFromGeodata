...
public class DQ_PositionalAccuracy {
    ...
    public DQ_PositionalAccuracy(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_PositionalAccuracy);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_PositionalAccuracy);
                if (!tempList.contains(elementName[i])) {
