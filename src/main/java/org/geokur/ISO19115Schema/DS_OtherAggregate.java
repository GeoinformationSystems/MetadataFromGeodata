...
public class DS_OtherAggregate extends DS_Aggregate {
    ...
    public DS_OtherAggregate(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DS_OtherAggregate);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DS_OtherAggregate);
                if (!tempList.contains(elementName[i])) {
