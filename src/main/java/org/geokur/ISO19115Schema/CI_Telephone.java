...
public class CI_Telephone {
    ...
    private final String[] elementName = {"number", "numberType"};
    ...
    public CI_Telephone(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.CI_Telephone);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.CI_Telephone);
                if (!tempList.contains(elementName[i])) {
