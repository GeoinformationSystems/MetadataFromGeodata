...
@XmlRootElement(name = "DQ_Homogeneity", namespace = "http://standards.iso.org/iso/19157/-2/mdq/1.0")
public class DQ_Homogeneity {
    ...
    private final int[] elementMax = {1, 1, 1, Integer.MAX_VALUE, 1, 1};
    ...
    public DQ_Homogeneity(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_Homogeneity);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_Homogeneity);
                if (!tempList.contains(elementName[i])) {
