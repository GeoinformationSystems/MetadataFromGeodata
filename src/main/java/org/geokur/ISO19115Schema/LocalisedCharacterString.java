...
@XmlRootElement(name = "LocalisedCharacterString", namespace = "http://standards.iso.org/iso/19115/-3/lan/1.0")
public class LocalisedCharacterString {

    // occurrence and obligation
    private final String[] elementName = {"locale"};
    ...
    public LocalisedCharacterString(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.LocalisedCharacterString);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.LocalisedCharacterString);
                if (!tempList.contains(elementName[i])) {
