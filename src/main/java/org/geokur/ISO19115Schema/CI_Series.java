...
public class CI_Series {
    ...
    private final String[] elementName = {"name", "issueIdentification", "page"};
    ...
    public CI_Series(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.CI_Series);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.CI_Series);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addIssueIdentification(String issueIdentification) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.issueIdentification.add(issueIdentification);
            }
        } catch (MaximumOccurrenceException | NoSuchFieldException | IllegalAccessException e) {
            System.out.println(e.getMessage());
        }
