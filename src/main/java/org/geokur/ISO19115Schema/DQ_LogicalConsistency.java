...
public class DQ_LogicalConsistency {
    ...
    public DQ_LogicalConsistency(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_LogicalConsistency);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_LogicalConsistency);
                if (!tempList.contains(elementName[i])) {
