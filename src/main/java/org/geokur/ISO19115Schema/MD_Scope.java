...
public class MD_Scope {

    // occurrence and obligation
    private final String[] elementName = {"level", "extent", "levelDescription"};
    ...
    // class variables
    @XmlElementWrapper(name = "level", namespace = "http://standards.iso.org/iso/19115/-3/mcc/1.0")
    @XmlElementRef
    public List<MD_ScopeCode> level;

    @XmlElementWrapper(name = "extent", namespace = "http://standards.iso.org/iso/19115/-3/mcc/1.0")
    @XmlElementRef
    public List<EX_Extent> extent;

    @XmlElementWrapper(name = "levelDescription", namespace = "http://standards.iso.org/iso/19115/-3/mcc/1.0")
    @XmlElementRef
    ...
    public MD_Scope(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_Scope);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_Scope);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addLevelDescription(MD_ScopeDescription levelDescription) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.levelDescription.add(levelDescription);
            }
        } catch (MaximumOccurrenceException | NoSuchFieldException | IllegalAccessException e) {
            System.out.println(e.getMessage());
        }
