...
public class DQ_ThematicAccuracy {
    ...
    public DQ_ThematicAccuracy(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_ThematicAccuracy);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_ThematicAccuracy);
                if (!tempList.contains(elementName[i])) {
