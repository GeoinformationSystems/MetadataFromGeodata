...
public class MD_RangeDimension {
    ...
    private final String[] elementName = {"sequenceIdentifier", "description", "name"};
    ...
    public MD_RangeDimension(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_RangeDimension);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_RangeDimension);
                if (!tempList.contains(elementName[i])) {
