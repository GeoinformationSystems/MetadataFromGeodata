...
public class MD_FeatureCatalogue extends MD_ContentInformation {
    ...
    public MD_FeatureCatalogue(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_FeatureCatalogue);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_FeatureCatalogue);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addFeatureCatalogue(String featureCatalogue) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.featureCatalogue.add(featureCatalogue);
            }
