...
public class DQ_TemporalQuality {
    ...
    public DQ_TemporalQuality(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_TemporalQuality);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_TemporalQuality);
                if (!tempList.contains(elementName[i])) {
