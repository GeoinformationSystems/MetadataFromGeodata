...
public class DQ_Metaquality {
    ...
    private final boolean[] elementObligation = {false, false, false, true, false, true};
    ...
    public DQ_Metaquality(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_Metaquality);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_Metaquality);
                if (!tempList.contains(elementName[i])) {
