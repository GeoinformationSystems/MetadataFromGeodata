...
public class MD_AttributeGroup {
    ...
    private final String[] elementName = {"contentType", "attribute"};
    ...
    public MD_AttributeGroup(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_AttributeGroup);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_AttributeGroup);
                if (!tempList.contains(elementName[i])) {
