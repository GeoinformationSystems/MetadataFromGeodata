...
@XmlRootElement(name = "MD_RepresentativeFraction", namespace = "http://standards.iso.org/iso/19115/-3/mri/1.0")
public class MD_RepresentativeFraction {

    // occurrence and obligation
    private final String[] elementName = {"denominator"};
    ...
    public MD_RepresentativeFraction(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_RepresentativeFraction);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_RepresentativeFraction);
                if (!tempList.contains(elementName[i])) {
