...
@XmlRootElement(name = "DQ_ConceptualConsistency", namespace = "http://standards.iso.org/iso/19157/-2/mdq/1.0")
public class DQ_ConceptualConsistency {
    ...
    public DQ_ConceptualConsistency(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_ConceptualConsistency);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_ConceptualConsistency);
                if (!tempList.contains(elementName[i])) {
