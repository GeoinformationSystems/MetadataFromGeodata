...
public class SV_Parameter {
    ...
    private final String[] elementName = {"name", "direction", "description", "optionality", "repeatability"};
    private final int[] elementMax = {1, 1, 1, 1, 1};
    private final boolean[] elementObligation = {true, true, false, true, true};

    private final String className = this.getClass().getSimpleName();
    private final boolean[] elementUsed = new boolean[elementName.length];

    // class variables
    @XmlElement(name = "name", namespace = "http://standards.iso.org/iso/19115/-3/srv/2.0")
    public List<String> name;

    @XmlElementWrapper(name = "direction", namespace = "http://standards.iso.org/iso/19115/-3/srv/2.0")
    @XmlElementRef
    public List<SV_ParameterDirection> direction;
    ...
    public SV_Parameter(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.SV_Parameter);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.SV_Parameter);
                if (!tempList.contains(elementName[i])) {
