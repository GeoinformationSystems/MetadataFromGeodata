...
@XmlRootElement(name = "DQ_QuantitativeAttributeAccuracy", namespace = "http://standards.iso.org/iso/19157/-2/mdq/1.0")
public class DQ_QuantitativeAttributeAccuracy {
    ...
    public DQ_QuantitativeAttributeAccuracy(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_QuantitativeAttributeAccuracy);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_QuantitativeAttributeAccuracy);
                if (!tempList.contains(elementName[i])) {
