...
public class EX_BoundingPolygon extends EX_GeographicExtent {
    ...
    public EX_BoundingPolygon(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.EX_BoundingPolygon);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.EX_BoundingPolygon);
                if (!tempList.contains(elementName[i])) {
