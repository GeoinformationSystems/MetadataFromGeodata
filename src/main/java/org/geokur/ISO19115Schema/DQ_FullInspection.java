...
public class DQ_FullInspection extends DQ_DataEvaluation {
    ...
    public DQ_FullInspection(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQ_FullInspection);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQ_FullInspection);
                if (!tempList.contains(elementName[i])) {
