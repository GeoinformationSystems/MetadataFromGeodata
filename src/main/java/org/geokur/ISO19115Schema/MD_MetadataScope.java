...
public class MD_MetadataScope {
    ...
    private final String[] elementName = {"resourceScope", "name"};
    ...
    public MD_MetadataScope(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MD_MetadataScope);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MD_MetadataScope);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addResourceScope(MD_ScopeCode resourceScope) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.resourceScope.add(resourceScope);
            }
