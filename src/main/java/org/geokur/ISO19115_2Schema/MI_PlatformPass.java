...
@XmlRootElement(name = "MI_PlatformPass", namespace = "http://standards.iso.org/iso/19115/-3/mac/2.0")
public class MI_PlatformPass {

    // occurrence and obligation
    private final String[] elementName = {"identifier", "extent", "relatedEvent"};
    ...
    @XmlElementWrapper(name = "relatedEvent", namespace = "http://standards.iso.org/iso/19115/-3/mac/2.0")
    @XmlElementRef
    public List<MI_Event> relatedEvent;
    ...
    public MI_PlatformPass(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MI_PlatformPass);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MI_PlatformPass);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addRelatedEvent(MI_Event relatedEvent) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.relatedEvent.add(relatedEvent);
            }
        } catch (MaximumOccurrenceException | NoSuchFieldException | IllegalAccessException e) {
            System.out.println(e.getMessage());
        }
