...
@XmlRootElement(name = "MI_RangeElementDescription", namespace = "http://standards.iso.org/iso/19115/-3/mrc/2.0")
public class MI_RangeElementDescription {

    // occurrence and obligation
    private final String[] elementName = {"name", "definition", "rangeElement"};
    ...
    // class variables
    @XmlElement(name = "name", namespace = "http://standards.iso.org/iso/19115/-3/mrc/2.0")
    ...
    public MI_RangeElementDescription(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.MI_RangeElementDescription);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.MI_RangeElementDescription);
                if (!tempList.contains(elementName[i])) {
    ...
    public void addRangeElement(Record rangeElement) {
        ...
        try {
            ...
            if (tempList.size() >= elementMax[elementNum]) {
                ...
                this.rangeElement.add(rangeElement);
            }
        } catch (MaximumOccurrenceException | NoSuchFieldException | IllegalAccessException e) {
            System.out.println(e.getMessage());
        }
