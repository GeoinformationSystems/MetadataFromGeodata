...
@XmlRootElement(name = "DQM_BasicMeasure", namespace = "http://standards.iso.org/iso/19157/-2/dqm/1.0")
public class DQM_BasicMeasure {

    // occurrence and obligation
    private final String[] elementName = {"name", "definition", "example", "valueType"};
    private final int[] elementMax = {1, 1, 1, 1};
    private final boolean[] elementObligation = {true, true, false, true};
    ...
    public DQM_BasicMeasure(){
        ...
        if (ProfileReader.profile != null) {
            for (int i = 0; i < elementName.length; i++) {
                List<String> tempList = Arrays.asList(ProfileReader.profile.used.DQM_BasicMeasure);
                if (!tempList.contains(elementName[i])) {
                    // element not used
                    elementUsed[i] = false;
                }
                tempList = Arrays.asList(ProfileReader.profile.obligation.DQM_BasicMeasure);
                if (!tempList.contains(elementName[i])) {
